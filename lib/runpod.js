...
async function cheapestAvailableGpuOrder() {
  ...
  try {
    ...
    // Secure Cloud always goes first, regardless of price: it's RunPod's own verified hardware, it's
    // the only cloud type the persistent network volume (pre-cached model) can attach to, and unlike
    // Community Cloud it can't be reclaimed by the host mid-request without warning -- that exact
    // failure mode (a pod that ran fine for minutes then vanished from the API with no crash on our
    // side) is what kept happening before this fix. Community Cloud is only a last-resort fallback,
    // sorted cheapest-first among itself, for when Secure Cloud has zero stock anywhere.
    combos.sort((a, b) => {
      if (a.cloudType !== b.cloudType) return a.cloudType === "SECURE" ? -1 : 1;
